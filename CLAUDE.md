...
| URL Surface | `API_PATHS` — every public path declared once, mounted by the API and requested by the SDK and dashboard. `/admin/*` is deliberately absent; see "Admin types". `DEPLOY_FIELDS` — the deploy multipart body's field names, the other half of the wire surface: three writers (SDK node + browser builders, n8n's hand-rolled client) restated them with no owner until 2.5.0-beta.21 |
