...
| Billing Types | `BillingInterval`, `Plan`, `PlansResponse`, `StripeSession` — the vocabulary only. No price and no cap is published: they are policy, delivered by `GET /plans` (see "Validation: format vs policy"). The platform runs on Stripe and its vocabulary says so (`StripeSession`); the plan vocabulary is fenced by `tests/billing-vocabulary.test.ts` |
